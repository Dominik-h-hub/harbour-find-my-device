...
# Literal lives in stdlib typing on Python >= 3.8 (Sailfish ships 3.8+); fall
# back to typing_extensions only if it is somehow missing.
try:
...
# Vendored trim: this app only needs the QR *matrix* (get_matrix()), never the
# image renderers. The upstream image backends pull in pypng / Pillow and a
# whole styles subtree, so they are intentionally NOT vendored. make_image() is
# therefore unavailable; BaseImage/PyPNGImage are stubs only for type hints.
BaseImage = object
PyPNGImage = None
